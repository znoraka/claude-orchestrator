...
/// Watches JSONL files and emits `jsonl-changed` events when they are modified.
/// Each watched path is debounced so rapid writes result in a single event.
...
impl JsonlWatcher {
            ...
            move |results: Result<Vec<notify_debouncer_mini::DebouncedEvent>, notify::Error>| {
                ...
                for event in events {
                    ...
                    if watched_set.contains(path) {
                        ...
                        let _ = app_handle.emit("jsonl-changed", &path_str);
                    }
                }
            },
        )
    ...
    /// Start watching a JSONL file. If the file's parent directory doesn't exist yet,
    /// we watch the grandparent (the projects dir) so we catch creation.
    pub fn watch(&mut self, path: PathBuf) -> Result<(), String> {
        ...
        // Watch the parent directory (the project directory) since the file may not exist yet
    ...
    pub fn unwatch(&mut self, path: &PathBuf) -> Result<(), String> {
        ...
        // Check if any other watched paths share the same parent directory
