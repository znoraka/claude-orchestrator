...
export default function Terminal({ sessionId, isActive, onExit, onTitleChange, onActivity, onRegisterWriteText, getPendingInput }: TerminalProps) {
  ...
  useEffect(() => {
    ...
    // Detect scroll-up to auto-lock, scroll-to-bottom to auto-unlock
    const handleWheel = (e: WheelEvent) => {
      if (e.deltaY < 0) {
        ...
        // Scrolling down — check if near bottom to unlock
  ...
  // Cmd+F to toggle search bar; Ctrl+Shift+S to toggle scroll lock
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.ctrlKey && e.shiftKey && e.key === "S" && isActive) {
        e.preventDefault();
        if (scrollLockedRef.current) {
          resumeScroll();
        } else {
  ...
  return (
    <div className="relative w-full h-full">
      <button
        ...
        className={`absolute bottom-4 right-4 z-10 flex items-center gap-1.5 bg-[var(--bg-secondary)] border rounded-lg px-2.5 py-1.5 shadow-lg text-xs transition-colors ${
          scrollLocked
            ? "border-yellow-500/50 text-yellow-400 hover:text-yellow-300 hover:border-yellow-400/70"
            : "border-[var(--border-color)] text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] hover:border-[var(--text-secondary)]"
        }`}
        title={scrollLocked ? "Resume output (Ctrl+Shift+S)" : "Pause output (Ctrl+Shift+S)"}
      >
        {scrollLocked ? (
          <>
